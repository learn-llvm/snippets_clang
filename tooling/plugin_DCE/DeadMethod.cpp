...
class DeclRemover : public RecursiveASTVisitor<DeclRemover> {
 ...
 private:
  ...
  void FlagMethodUsed(const CXXMethodDecl *m) {
    if (!m || !(m = m->getCanonicalDecl())) return;
...
class DeclCollector : public RecursiveASTVisitor<DeclCollector> {
 public:
  DeclCollector(ASTContext &c, ClassSet &u, MethodSet &p, bool t, FileList &b)
      : ctx(c),
        undefinedClasses(u),
        privateMethods(p),
        templates(t),
  ...
  bool VisitCXXMethodDecl(CXXMethodDecl *m) {
    ...
    if (!m->isDefined()) MarkUndefined(r);

    // only private methods are concerned
    if (m->getAccess() != AS_private) return true;

    // omit template methods if flag on
    if (IsTemplated(m) && !templates) return true;

    // omit blacklist entries
    if (IsBlacklisted(m)) return true;
  ...
  bool VisitCXXRecordDecl(CXXRecordDecl *r) {
    ...
    if (r && !r->hasDefinition()) MarkUndefined(r);
 ...
 private:
  ...
  bool IsTemplated(const CXXMethodDecl *m) {
    if (m->getDescribedFunctionTemplate()) return true;
    return false;
  }
  ...
  bool IsBlacklisted(const CXXMethodDecl *m) {
    ...
    FileList::const_iterator it =
        std::lower_bound(blacklist.begin(), blacklist.end(), file);
...
class DeadConsumer : public ASTConsumer {
 ...
 private:
  ...
  void WarnUnused(ASTContext &ctx, ClassSet &undefined, MethodSet &unused) {
    ...
    for (MethodSet::iterator I = unused.begin(), E = unused.end(); I != E;
         ++I) {
      const CXXMethodDecl *m = *I;

      // care only about fully defined classes
      if (!IsDefined(ctx, undefined, m->getParent())) continue;
  ...
  // are all defined
  bool IsDefined(ASTContext &ctx, ClassSet &undefined, const CXXRecordDecl *r) {
    if (Contains(ctx, undefined, ctx.getRecordType(r))) return false;
    ...
    for (CXXRecordDecl::friend_iterator I = r->friend_begin(),
      ...
      if (fFun) {
        if (!fFun->getCanonicalDecl()->isDefined()) return false;
      }

      // ...or a type
      const TypeSourceInfo *fInfo = (*I)->getFriendType();
      if (fInfo) {
        if (Contains(ctx, undefined, fInfo->getType())) return false;
      }
    }
  ...
  void MakeUnusedWarning(DiagnosticsEngine &diags, const CXXMethodDecl *m) {
    unsigned diagId = diags.getCustomDiagID(
        DiagnosticsEngine::Warning, "private method %0 seems to be unused");
...
class DeadAction : public PluginASTAction {
 ...
 private:
  ...
  void ShowHelp() {
    llvm::errs()
        << "DeadMethod plugin: warn if fully defined classes "
...
// register the plugin
static FrontendPluginRegistry::Add<DeadAction> X(
    "dead-method", "look for unused private methods");
