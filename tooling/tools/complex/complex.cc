...
AST_MATCHER_P(DeclStmt, containsDeclarationAnywhere, internal::Matcher<Decl>,
              InnerMatcher) {
...
AST_MATCHER_P(CXXOperatorCallExpr, operatorOn, internal::Matcher<Expr>,
              InnerMatcher) {
  const Expr *ExprNode = const_cast<CXXOperatorCallExpr &>(Node)
                             .getCallee()
                             ->IgnoreParenImpCasts();
  return (ExprNode != NULL && InnerMatcher.matches(*ExprNode, Finder, Builder));
}
}
}
...
class DeclarationFixerCallback : public MatchFinder::MatchCallback {
 public:
  DeclarationFixerCallback(Replacements *replacements, bool use_pyopencl_types)
      : _replacements(replacements), _use_pyopencl_types(use_pyopencl_types) {}
  ...
  virtual void run(const MatchFinder::MatchResult &Result) {
    ...
    string type_text = getText(*Result.SourceManager, replacement_location);

    // Adjust it appropriately
    string replacement_text = transform_complex_type_declaration(type_text);

    // Apply the replacement
    _replacements->insert(Replacement(*Result.SourceManager,
                                      &replacement_location, replacement_text));
  }
...
class ConstructorFixerCallback : public MatchFinder::MatchCallback {
 public:
  ConstructorFixerCallback(Replacements *replacements, bool use_pyopencl_types)
      : _replacements(replacements), _use_pyopencl_types(use_pyopencl_types) {}

  string transform_construction_expression(
      string original_expression, const CXXConstructExpr *constructor) {
    ...
    if (_use_pyopencl_types) {
      ...
      QualType template_type =
          template_decl->getTemplateArgs().get(0).getAsType();
      new_literal = "c" + template_type.getAsString() + "_new(" + real_literal +
                    ", " + imag_literal + ")";
    } else {
      new_literal =
  ...
  virtual void run(const MatchFinder::MatchResult &Result) {
    ...
    // Grab the text which specifies the type
    string construction_text = getText(*Result.SourceManager, *constructor);
    ...
    string replacement_text =
        transform_construction_expression(construction_text, constructor);

    // Apply the replacement
    _replacements->insert(
        Replacement(*Result.SourceManager, constructor, replacement_text));
  }
 ...
 public:
  ...
  virtual void run(const MatchFinder::MatchResult &Result) {
    ...
    string operator_text =
        getText(*Result.SourceManager, *overloaded_operator->getCallee());

    // If it's not something we need to replace, ignore it
    if (operator_text != "+" && operator_text != "-" && operator_text != "*" &&
        operator_text != "/") {
      return;
    }
    ...
    SourceLocation start_location = Result.SourceManager->getSpellingLoc(
        overloaded_operator->getArg(0)->getLocStart());
    SourceLocation mid_location = Result.SourceManager->getSpellingLoc(
    ...
    bool left_is_real =
        overloaded_operator->getArg(0)->getType().getAsString().find('<') ==
        string::npos;
    bool right_is_real =
        overloaded_operator->getArg(1)->getType().getAsString().find('<') ==
        string::npos;
    ...
    printf("Fixing: %s (%i (%s) %i (%s))\n",
           ...
           left_is_real,
           overloaded_operator->getArg(0)->getType().getAsString().c_str(),
           right_is_real,
           overloaded_operator->getArg(1)->getType().getAsString().c_str());
    ...
    if (operator_text == "+") {
      if (left_is_real) {
        ...
        start_insert = "c" + base_type_string + "_radd(";
      } else if (right_is_real) {
        // Left is complex, right is real
        start_insert = "c" + base_type_string + "_addr(";
      } else {
        // Both are complex
        start_insert = "c" + base_type_string + "_add(";
      }
      ...
      if (left_is_real) {
        // Left is real, right is complex
        start_insert = "c" + base_type_string + "_rmul(";
      } else if (right_is_real) {
        // Left is complex, right is real
        start_insert = "c" + base_type_string + "_mulr(";
      } else {
        // Both are complex
        start_insert = "c" + base_type_string + "_mul(";
      }
    } else if (operator_text == "/") {
      if (left_is_real) {
        // Left is real, right is complex
        start_insert = "c" + base_type_string + "_rdivide(";
      } else if (right_is_real) {
        // Left is complex, right is real
        start_insert = "c" + base_type_string + "_divider(";
      } else {
        // Both are complex
        start_insert = "c" + base_type_string + "_divide(";
      }
    }
    ...
    for (Replacements::iterator rit = _replacements->begin();
      ...
      if (start_insertion.getFilePath().compare(rit->getFilePath()) == 0 &&
        ...
        start_insertion =
            Replacement(*Result.SourceManager, start_location, 0, start_insert);
    ...
    _replacements->insert(
        Replacement(*Result.SourceManager, mid_location,
                    1,  // All our operators should only be 1 character
                    mid_replacement));
    _replacements->insert(
        Replacement(*Result.SourceManager, end_location, 0, end_insert));
  }
...
class MemberFunctionFixerCallback : public MatchFinder::MatchCallback {
 public:
  MemberFunctionFixerCallback(Replacements *replacements,
  ...
  virtual void run(const MatchFinder::MatchResult &Result) {
    ...
    bool is_dereference = cast<MemberExpr>(member_call->getCallee())->isArrow();
    ...
    string member_name = member_call->getMethodDecl()->getNameAsString();
    ...
    string complete_expression = getText(*Result.SourceManager, *member_call);

    // Grab the text of just called variable
    string called_variable = getText(*Result.SourceManager, *call_variable);

    // Determine the length of stuff trailing after the variable
    unsigned tail_length = complete_expression.size() - called_variable.size();

    // Calculate start/end insertion location
    SourceLocation start_location =
        Result.SourceManager->getSpellingLoc(call_variable->getLocStart());
    SourceLocation end_location =
        start_location.getLocWithOffset(called_variable.size());

    // Set up the insertions/replacements
    _replacements->insert(
        Replacement(*Result.SourceManager, start_location, 0, prefix));
    _replacements->insert(
        Replacement(*Result.SourceManager, end_location, tail_length, suffix));

    printf("Found member function call on %s [%s] (%i)\n%s\n%s\n",
           getText(*Result.SourceManager, *call_variable).c_str(),
           getText(*Result.SourceManager,
                   *cast<MemberExpr>(member_call->getCallee())).c_str(),
           is_dereference, prefix.c_str(), suffix.c_str());
  }
...
class StaticFunctionFixerCallback : public MatchFinder::MatchCallback {
 public:
  StaticFunctionFixerCallback(Replacements *replacements,
                              bool use_pyopencl_types)
      : _replacements(replacements), _use_pyopencl_types(use_pyopencl_types) {}
...
// Set up the custom command line options
...
int main(int argc, const char **argv) {
  ...
  cl::OptionCategory OC("hello");
  CommonOptionsParser OptionsParser(argc, argv, OC);
  ...
  MemberFunctionFixerCallback member_function_fixer(&Tool.getReplacements(),
                                                    use_pyopencl_types);
  StaticFunctionFixerCallback static_function_fixer(&Tool.getReplacements(),
                                                    use_pyopencl_types);

  // Add all the matchers we are interested in
  Finder.addMatcher(
      id("declaration", varDecl(hasType(asMatchingString(".*complex<.*>.*")))),
      &declaration_fixer);
  Finder.addMatcher(
      id("declaration",
         functionDecl(returns(asMatchingString(".*complex<.*>.*")))),
      &declaration_fixer);
  Finder.addMatcher(id("constructor", constructExpr(hasDeclaration(methodDecl(
                                          ofClass(hasName("std::complex")))))),
                    &constructor_fixer);
  if (use_pyopencl_types) {
    Finder.addMatcher(
        id("overloaded_operator", operatorCallExpr(operatorOn(hasType(
                                      asMatchingString(".*complex<.*>.*"))))),
        &operator_fixer);
  }
  Finder.addMatcher(
      id("member_call",
         memberCallExpr(on(hasType(asMatchingString(".*complex<.*>.*"))))),
      &member_function_fixer);
  Finder.addMatcher(
      id("static_call",
         callExpr(argumentCountIs(1), hasArgument(0, hasType(asMatchingString(
                                                         ".*complex<.*>.*"))))),
      &static_function_fixer);

  // TODO: Run through the list of files and replace "#include <complex>" with
  // "#include <complex.h>"
