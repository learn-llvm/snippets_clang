...
const char *_getCompletionAvailabilitySpelling(
    CXAvailabilityKind availavility) {
  switch (availavility) {
...
void show_completion_results(CXCodeCompleteResults *compResults) {
  ...
  CXCursorKind kind =
      clang_codeCompleteGetContainerKind(compResults, &isIncomplete);
  printf("Complete: %d\n", !isIncomplete);
  ...
  for (auto i = 0U; i < compResults->NumResults; i++) {
    ...
    CXAvailabilityKind availavility =
        clang_getCompletionAvailability(compString);
    const char *availavilityText =
        _getCompletionAvailabilitySpelling(availavility);
    ...
    for (auto j = 0U; j < numChunks; j++) {
      ...
      // CXCompletionString child =
      // clang_getCompletionChunkCompletionString(compString);
...
int main(int argc, char **argv) {
  ...
  CXTranslationUnit tu = clang_parseTranslationUnit(
      index, filename, cmdArgs, numArgs, NULL, 0,
  ...
  // Code Completion
  CXCodeCompleteResults *compResults;
  compResults = clang_codeCompleteAt(tu, filename, lineno, columnno, NULL, 0,
