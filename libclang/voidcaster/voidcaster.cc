...
#ifdef GCC_SYSINCLUDE
      "  -g                     don't add the include path of the installed "
      "GCC\n"
      "                         automatically\n"
#endif
      "  -i                     interactive mode\n"
      "  -I<path>               add a path where the preprocessor shall "
      "search\n"
      "                         for includes\n"
 ...
 */
static void warnMissingVoid(const char *file, const char *func,
                            module_loc_t loc) {
  (void)fprintf(stderr,
 ...
 */
static void warnSuperfluousVoid(const char *file, const char *func,
                                module_loc_t start, module_loc_t end) {
  (void)fprintf(
      stderr, "%s:%zu:%zu: Pointless cast to void when calling function %s.\n",
...
static inline void pointless(const char *option) {
  (void)fprintf(stderr,
                "Warning: it is pointless to specify %s multiple times.\n",
                option);
}
...
int main(int argc, char **argv) {
...
#ifdef GCC_SYSINCLUDE
      case 'g':
        if (!inclgcc) pointless("-g");
        inclgcc = false;
        break;
#endif
      case 'i':
        if (interactive) pointless("-i");
        interactive = true;
        break;
      case 's':
        if (extstatus) pointless("-s");
        extstatus = true;
        break;
      case '?':
        usage();
  ...
  for (i = optind; i < argc; ++i) {
    ret = processFile(idx, argv[i], clangargs.count,
                      (const char **)clangargs.arr, missProc, superProc);
