 ...
 */
static inline void cursorLocation(CXCursor cur, CXString *locFileName,
                                  module_loc_t *loc) {
  unsigned int l, c;
 ...
 */
static inline void castExtent(CXCursor cur, module_loc_t *start,
                              module_loc_t *end) {
  CXToken *toks;
  CXCursor *curs;
  ...
  /* annotate it */
  curs = static_cast<CXCursor *>(malloc(numToks * sizeof(CXCursor)));
  if (curs == NULL) {
    perror("malloc");
    exit(EXITCODE_MM);
  }
  ...
  for (i = 0; i < numToks; ++i) {
    ...
    if (clang_getCursorKind(cur) != clang_getCursorKind(curs[i]) ||
        !clang_equalTypes(clang_getCursorType(cur),
                          clang_getCursorType(curs[i]))) {
    ...
    if (!startSet) {
      ...
      clang_getPresumedLocation(clang_getRangeStart(tokExt), NULL, &newSL,
                                &newSC);
      start->line = newSL;
      start->col = newSC;
      startSet = true;
 ...
 */
static enum CXChildVisitResult visitation(CXCursor cur, CXCursor parent,
                                          CXClientData dta) {
  CXString locFileName;
  bool disposeFileName = false;

  descent_state *dstate = (descent_state *)dta;
  descent_state kiddstate = {.missProc = dstate->missProc,
  ...
  if (curKind == CXCursor_CompoundStmt || curKind == CXCursor_CaseStmt) {
    ...
    if (clang_getCursorType(cur).kind == CXType_Void) {
      ...
      /* fetch location info */
      castExtent(cur, &kiddstate.castLoc.startLoc, &kiddstate.castLoc.endLoc);
    ...
    if (clang_Cursor_isNull(target) ||
        clang_equalLocations(clang_getCursorLocation(cur),
                             clang_getCursorLocation(target))) {
      /* function decl not found */
      (void)fprintf(stderr,
                    "%s:%zu:%zu: Warning: can't check call to %s (can't find "
                    "original definition).\n",
      ...
      switch (clang_getCursorResultType(target).kind) {
        case CXType_Void:
          if (dstate->voidCastAbove) {
            /* magic! */
            dstate->superProc(clang_getCString(locFileName),
                              clang_getCString(funcName),
                              dstate->castLoc.startLoc, dstate->castLoc.endLoc);
          }
          break;
        case CXType_Invalid:
        ...
        default:
          if (dstate->compoundStmtAbove && !dstate->voidCastAbove) {
            dstate->missProc(clang_getCString(locFileName),
                             clang_getCString(funcName), loc);
          }
          break;
      }
    }
    ...
    (void)printf(
        ...
        dstate->level, clang_getCString(cursKind), clang_getCString(cursDesc),
        clang_getCString(locFileName), loc.line, loc.col);
  ...
  (void)clang_visitChildren(cur, visitation, (CXClientData)&kiddstate);

  if (disposeFileName) clang_disposeString(locFileName);
...
enum exitcodes_e processFile(CXIndex idx, const char *filename,
                             ...
                             superfluousVoidProc superProc) {
  unsigned int i;
  descent_state dstate = {.missProc = missProc,
                          .superProc = superProc,
                          .level = 0,
                          .voidCastAbove = false,
                          .compoundStmtAbove = false};
  ...
  (void)clang_visitChildren(clang_getTranslationUnitCursor(tu), visitation,
                            (CXClientData)&dstate);
