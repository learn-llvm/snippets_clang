...
static inline module_loc_t modifCharacteristicLoc(const modif_t *mod) {
  switch (mod->type) {
    case modif_s::MODIF_INSERT:
      return mod->m.insert.where;
    case modif_s::MODIF_REMOVE:
...
static bool addModif(modif_t toadd) {
  ...
  newModifs = static_cast<modif_t *>(
      realloc(modifs, (numModifs + 1) * sizeof(modif_t)));
  if (newModifs == NULL) {
    /* that went belly-up */
    perror("realloc");
    return false;
 ...
 */
static void fetchFileLines(const char *file, size_t linenum, size_t linecount,
                           char **lines, size_t *lineslen) {
  int fd;
  char *f;
  ...
  *lineslen = 0;
  ...
  /* map it into memory */
  f = (char *)mmap(NULL, filelen, PROT_READ, MAP_PRIVATE, fd, 0);
  if (f == MAP_FAILED) {
    perror("mmap");
    (void)close(fd);
    *lines = NULL;
  ...
  *lines = (char *)malloc(((*lineslen) + 1) * sizeof(char));
  if (*lines == NULL) {
    perror("malloc");
    (void)munmap(f, filelen);
    (void)close(fd);
...
void interactMissingVoid(const char *file, const char *func, module_loc_t loc) {
  ...
  (void)printf(
      ...
      "%.*s(void)%s\n"
      "Apply fix? (y/n) ",
      file, loc.line, func, line, (int)loc.col - 1, line, line + loc.col - 1);
  (void)fflush(stdout);
  ...
  if (fetchBoolResponse()) {
    ...
    modif_t newFix = {
        .file = strdup(file),
        .type = modif_s::MODIF_INSERT,
        .m = {.insert = {.where = loc, .what = strdup("(void)")}}};
...
void interactSuperfluousVoid(const char *file, const char *func,
  ...
  (void)printf(
      ...
      "%.*s%s\n"
      "Apply fix? (y/n) ",
      file, start.line, end.line, func, lines, (int)startOffset, lines,
      lines + endOffset + 1);
  (void)fflush(stdout);

  if (fetchBoolResponse()) {
    /* queue the fix */
    modif_t newFix = {.file = strdup(file),
                      .type = modif_s::MODIF_REMOVE,
                      .m = {.remove = {.fromWhere = start, .toWhere = end}}};
...
static inline void overwriteWithBackup(const char *oldP, const char *newP) {
  ...
  char *backFn = (char *)malloc(strlen(oldP) + 2);
  if (backFn == NULL) {
    perror("malloc");
    return;
  }
 ...
 */
static bool moveFileUntil(FILE *rf, module_loc_t now, module_loc_t target,
                          FILE *wf) {
  int c;

  while (now.line < target.line ||
         (now.line == target.line && now.col < target.col)) {
    c = fgetc(rf);
    if (c == EOF) {
      /* reached end-of-file... */
      return false;
...
void performModifs(void) {
  ...
  int tmpfd;
  module_loc_t curLoc = {.line = 1, .col = 1};
  ...
  for (i = 0; i < numModifs; ++i) {
    ...
    if (strcmp(rFn, modifs[i].file) != 0) {
      ...
      if (rf != NULL && wf != NULL) copyRest(rf, wf);
      ...
      (void)snprintf(tmpfn, sizeof(tmpfn) / sizeof(tmpfn[0]),
                     "/tmp/voidcasterXXXXXX");
      tmpfd = mkstemp(tmpfn);
      if (tmpfd == -1) {
        perror("mkstemp");
        (void)fclose(rf);
    ...
    switch (modifs[i].type) {
      ...
      case modif_s::MODIF_REMOVE:
        ...
        if (!moveFileUntil(rf, modifs[i].m.remove.fromWhere,
                           modifs[i].m.remove.toWhere, NULL)) {
          /* crud. */
          (void)fprintf(stderr, "I/O troubles...\n");
          (void)fclose(rf);
          (void)fclose(wf);
  ...
  if (rf != NULL && wf != NULL) copyRest(rf, wf);
  if (rf != NULL) (void)fclose(rf);
  if (wf != NULL) (void)fclose(wf);
