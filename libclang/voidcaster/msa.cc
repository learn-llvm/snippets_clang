...
int msa_create(msa_t *msa)
{
	...
	msa->count = 0;
	msa->arr = static_cast<char**>(malloc(msa->capacity * sizeof(char *)));
	if (msa->arr == NULL)
	{
		/* malloc failed */
		msa->capacity = 0;
...
int msa_add(msa_t *msa, const char *str)
{
	...
	if (msa->count >= msa->capacity)
	{
		...
		newarr = static_cast<char**>(realloc(msa->arr, msa->capacity * sizeof(char *)));
		if (newarr == NULL)
		{
...
int msa_add_prefixed(msa_t *msa, const char *fst, const char *snd)
{
	...
	char *newstr = static_cast<char*>(malloc(newsize));
	if (newstr == NULL)
	{
		/* malloc failed */
		return 0;
